...
const SeeMoreCompletedJob = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      try {
        const response = await apiConnector("GET", `${jobEndPoints.GETCOMPLETEDJOB_API}/${jobId}`);
  ...
  return (
    <div className="my-10 flex flex-col gap-y-4 rounded-md border border-richblack-700 bg-richblack-800 p-8 px-12 text-richblack-25">
      ...
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm px-4 py-1 bg-caribbeangreen-200 text-black font-semibold rounded-md">
          Completed
        </span>
      </div>
        ...
        <div className='flex gap-2'>
            ...
            <button
                ...
                className='flex gap-1 items-center hover:scale-105 bg-[#c90b0b] py-2 px-5 font-semibold text-richblack-900 rounded-md'>
                  Create a dispute<IoIosWarning size={19}/>
            </button>
        </div>
      </div> */}
...
export default SeeMoreCompletedJob;
